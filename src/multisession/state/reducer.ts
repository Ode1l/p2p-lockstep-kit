...
import type { OrderedEvent } from "./events";
...
>(
  ...
  if (event.hostPeerId !== state.hostPeerId) {
    return failure("event has wrong hostPeerId");
  }
  ...
  if (event.type === "MEMBERSHIP_JOINED") {
    if (event.actorId !== state.hostId) {
      return failure("only host can admit a participant");
    }
  ...
  if (event.type === "PEER_BINDING_UPDATED") {
    if (event.actorId !== state.hostId) {
      return failure("only host can update a peer binding");
    }
    const participantId = value.participantId as ParticipantId;
    const peerId = value.peerId as PeerId;
  ...
  if (event.type === "DISPLAY_NAME_CHANGED") {
    const participantId = value.participantId as ParticipantId;
    if (event.actorId !== participantId) {
      return failure("display-name actor mismatch");
    }
    ...
    if (!participant) return failure("participant does not exist");
    const participants = new Map(state.participants).set(participantId, {
      ...participant,
      displayName,
    });
    return success(withEventPosition({ ...state, participants }, event));
  }
  ...
  if (event.type === "GAME_STARTED") {
    if (event.actorId !== state.hostId) return failure("only host can start");
    ...
    const game = plugin.createInitialState({
      ...
      participants: state.participants,
      seats: state.seats,
      startPayload: value as JsonObject,
    });
    return success(
      withEventPosition(
        {
  ...
  if (event.type === "GAME_RESTARTED") {
    ...
    if (event.actorId !== state.hostId) return failure("only host can restart");
