...
const configPath = fileURLToPath(new URL("../configuration.json", import.meta.url));
const fileConfig = JSON.parse(readFileSync(configPath, "utf8")) as ServerConfig;
const signalingHost = process.env.SIGNALING_HOST ?? fileConfig.signalingHost;
const signalingPort = Number(process.env.SIGNALING_PORT ?? fileConfig.signalingPort);
const iceServers = fileConfig.iceServers;
